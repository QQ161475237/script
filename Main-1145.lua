...
end    ParticleContainer.Parent = Splash
...
ForsakenMerzzLBox:AddButton({
    ...
    Func = function()
        ...
        Library:Notify({
            Title = "MerzzL",
            Description = "Forsaken MerzzL 代码已复制",
            Duration = 2
        })
    end
})
...
PlayerLeft:AddToggle("NoclipToggle", {
	...
	Callback = function(v)
        ...
        Library:Notify({
            Title = "MerzzL",
            Description = v and "穿墙已开启" or "穿墙已关闭（仅头+胸碰撞）",
            Duration = 2
        })
	end
})
...
PlayerRight:AddButton({
    ...
    Func = function()
        ...
        Library:Notify({
            Title = "MerzzL",
            Description = "踏空行走脚本已加载",
            Duration = 3
        })
    end
})

-- 视觉标签：全套ESP + 地图视觉 + 简易ESP + 亮度/无阴影/除雾
local ESPFuncBox = Tabs.ESP:AddLeftGroupbox("全套ESP开关")
local VisionBox = Tabs.ESP:AddLeftGroupbox("地图视觉设置")
local ESPBox = Tabs.ESP:AddRightGroupbox("简易ESP")
...
VisionBox:AddToggle("MapNightVisionTog", {
	...
	Callback = function(v)
        ...
        Library:Notify({
            Title = "MerzzL",
            Description = v and "地图夜视已开启" or "地图夜视已关闭",
            Duration = 2
        })
	end
})
...
OtherBox:AddToggle("InvAlwaysShowTog", {
	...
	Callback = function(v)
        ...
        Library:Notify({
            Title = "MerzzL",
            Description = v and "物品栏始终显示 已开启" or "物品栏始终显示 已关闭",
            Duration = 2
        })
	end
})
...
task.spawn(function()
	...
	local function SetCollision(character, state)
		for _, part in pairs(character:GetDescendants()) do
			if part:IsA("BasePart") then
				if state then
					part.CanCollide = table.find(CollisionParts, part.Name) ~= nil
				else
					part.CanCollide = false
				end
			end
	...
	RunService.Heartbeat:Connect(function()
		if not lp.Character then return end
		SetCollision(lp.Character, not _G.NoclipActive)
        ...
        if _env.NoFog ~= nil then
            Lighting.FogEnd = _env.NoFog and 1000000 or origFog
        end
